...
# Modified work Copyright (c) 2025 wigum
...
class COCOeval_faster(COCOevalBase):
    ...
    def extended_metrics(self):
        ...
        precision, recall, F1-score, and multiple accuracy variants. Results are computed using evaluation results
        ...
        Returns:
            dict: A dictionary with the following keys:
                - 'class_map' (list of dict): List of per-class and overall metrics, each as a dictionary containing:
                    ...
                    - 'f1' (float): F1-score for the class (at selected recall threshold) or macro F1 for "all".
                    - 'tp_ratio' (float): TP/(TP+FP+FN) (no TN; legacy convenience measure for detection).
                    - 'image_accuracy' (float): Image-level accuracy with TN = (TP + TN) / (TP + TN + FP + FN),
                      where TP/FP/FN/TN are computed over images for that class. A class-image is positive if the
                      image has at least one GT of the class; it's predicted positive if there is at least one
                      detection of that class; a true positive image has at least one matched TP detection.
                ...
                - 'tp_ratio' (float): Macro-averaged TP ratio (mean over classes of TP/(TP+FP+FN)).
                - 'image_accuracy' (float): Macro-averaged image-level accuracy using TN definition above.
        ...
        Notes:
            ...
            - 'tp_ratio' excludes true negatives (classical TN ill-defined on object instances) and is retained
              for continuity; 'image_accuracy' supplies a (TP+TN)/(TP+TN+FP+FN) style metric at image granularity.
        """
        ...
        denom_macro_f1 = (macro_precision + macro_recall)
        macro_f1 = float(2 * macro_precision * macro_recall / denom_macro_f1) if denom_macro_f1 > 0 else 0.0
        ...
        # Optional: compute per-class instance-level TP/FP/FN (legacy tp_ratio) and image-level confusion (needs self.matched)
        tp_counts, fp_counts, fn_counts = {}, {}, {}
        image_level = {}
        if getattr(self, "matched", False) and self.params.useCats:
            # Precompute per-class sets of positive (GT) images
            class_positive_imgs = {}
            for _gt in self.cocoGt.anns.values():
                cid = _gt.get("category_id")
                img_id = _gt.get("image_id")
                class_positive_imgs.setdefault(cid, set()).add(img_id)
            # Collect detection info
            dt_by_class_imgs = {}
            tp_by_class_imgs = {}
            for _dt in self.cocoDt.anns.values():
                cid = _dt.get("category_id")
                img_id = _dt.get("image_id")
                if _dt.get("tp"):
                    tp_counts[cid] = tp_counts.get(cid, 0) + 1
                    tp_by_class_imgs.setdefault(cid, set()).add(img_id)
                elif _dt.get("fp"):
                    fp_counts[cid] = fp_counts.get(cid, 0) + 1
                # Count all detections (tp or fp) toward predicted-positive images
                dt_by_class_imgs.setdefault(cid, set()).add(img_id)
            # FN collection (from unmatched GTs)
            for _gt in self.cocoGt.anns.values():
                if _gt.get("fn"):
                    cid = _gt.get("category_id")
                    fn_counts[cid] = fn_counts.get(cid, 0) + 1
            # Build image-level confusion counts per class
            all_img_ids = set(self.params.imgIds) if getattr(self.params, 'imgIds', None) else set()
            for cid, pos_imgs in class_positive_imgs.items():
                pred_imgs = dt_by_class_imgs.get(cid, set())
                tp_imgs = tp_by_class_imgs.get(cid, set())
                neg_imgs = all_img_ids - pos_imgs
                # Image-level confusion
                tp_i = len(tp_imgs)
                fn_i = len(pos_imgs - tp_imgs)
                fp_i = len((neg_imgs) & pred_imgs)
                tn_i = len(neg_imgs - pred_imgs)
                denom_i = tp_i + fp_i + fn_i + tn_i
                image_acc = (tp_i + tn_i) / denom_i if denom_i > 0 else float('nan')
                image_level[cid] = {
                    'image_tp': tp_i,
                    'image_fp': fp_i,
                    'image_fn': fn_i,
        ...
        if self.params.useCats:
            ...
            for k, cid in enumerate(cat_ids):
                ...
                # Per-class F1 (at selected recall threshold)
                f1_c = float(2 * pc * rc / (pc + rc)) if (pc + rc) > 0 else float("nan")

                # Per-class tp_ratio (instance-level, no TN)
                tp_ratio_c = float("nan")
                if tp_counts or fp_counts or fn_counts:
                    tp_c = tp_counts.get(cid, 0)
                    fp_c = fp_counts.get(cid, 0)
                    fn_c = fn_counts.get(cid, 0)
                    denom_c = tp_c + fp_c + fn_c
                    if denom_c > 0:
                        tp_ratio_c = tp_c / denom_c

                image_acc_c = image_level.get(cid, {}).get('image_accuracy', float('nan'))
                ...
                per_class.append({
                    ...
                    "f1": f1_c,
                    "tp_ratio": tp_ratio_c,
                    "image_accuracy": image_acc_c,
                })

        # Add metrics for all classes combined
        # Macro tp_ratio (mean of per-class tp_ratio)
        valid_tp_ratio = [c["tp_ratio"] for c in per_class if isinstance(c.get("tp_ratio"), float) and not np.isnan(c["tp_ratio"])]
        macro_tp_ratio = float(np.mean(valid_tp_ratio)) if len(valid_tp_ratio) else float("nan")
        # Macro image-level accuracy
        valid_img_acc = [c["image_accuracy"] for c in per_class if isinstance(c.get("image_accuracy"), float) and not np.isnan(c["image_accuracy"])]
        macro_image_accuracy = float(np.mean(valid_img_acc)) if len(valid_img_acc) else float("nan")
        ...
        per_class.append({
            ...
            "recall": macro_recall,
            "f1": macro_f1,
            "tp_ratio": macro_tp_ratio,
            "image_accuracy": macro_image_accuracy,
        })
